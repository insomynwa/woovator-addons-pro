...
<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<table class="wv_cart_table shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
		<thead>
			<tr>
				<?php
					foreach ( $cartitem as $itemvalue ) {
						if( $itemvalue['table_items'] == 'customadd' ){
							echo '<th class="product-'.esc_attr( uniqid('wvcustomitem_') ).' elementor-repeater-item-'.$itemvalue['_id'].'">'.esc_html( $itemvalue['table_heading_title'] ).'</th>';
						}else{
						...
						<?php
							foreach ( $cartitem as $itemvalue ) {
								...
								switch ( $itemvalue['table_items'] ) {
									...
									case 'remove':
										echo '<td class="product-remove elementor-repeater-item-'.$itemvalue['_id'].'">';
											echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
												...
												esc_html__( 'Remove this item', 'woovator-pro' ),
												esc_attr( $product_id ),
									...
									case 'name':
										echo '<td class="product-name elementor-repeater-item-'.$itemvalue['_id'].'" data-title=" '.$itemvalue['table_heading_title'].' ">';
											...
											if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
												echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'woovator-pro' ) . '</p>', $product_id ) );
											}
										echo '</td>';
										break;
									...
									case 'customadd':
										echo '<td class="product-wvcustomdata elementor-repeater-item-'.$itemvalue['_id'].'">';
											$cart_custom_data = get_post_meta( $product_id, 'woovator_cart_custom_content', true );
			...
			<tr>
				<td colspan="<?php echo count( $cartitem ); ?>" class="actions">
					...
					<?php if ( wc_coupons_enabled() && $cartopt['coupon_form']['enable'] === 'yes' ) { ?>
						<div class="coupon">
							<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( $cartopt['coupon_form']['placeholder'], 'woovator-pro' ); ?>" /> <input type="submit" class="button" name="apply_coupon" value="<?php esc_attr_e( $cartopt['coupon_form']['button_txt'], 'woovator-pro' ); ?>" />
							<?php do_action( 'woocommerce_cart_coupon' ); ?>
						</div>
					<?php } ?>

					<div class="wv_update_cart_shop">
						<?php if( $cartopt['continue_shop_button']['enable'] === 'yes' ){ ?>
							<a class="wvbutton-continue-shopping"  href="<?php echo esc_url( apply_filters( 'woocommerce_continue_shopping_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
								<?php echo ( is_rtl() ? '&#8594;' : '&#8592;' ) . '&nbsp;' . esc_html__( $cartopt['continue_shop_button']['button_txt'], 'woovator-pro' ); ?>
							</a>
						<?php } ?>
