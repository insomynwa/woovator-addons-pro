<?php
	class WooVatorProBase {
    	public $key = "4DEB1BC6E89224E1";
    	private $product_id = "2";
    	private $product_base = "woovator-pro";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_woovator-pro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=woovator-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
