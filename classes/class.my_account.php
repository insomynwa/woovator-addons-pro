<?php
    ...
    public function navigation_user(){
        ...
        if ( $current_user->display_name ) {
            ...
            $name = esc_html__( 'Welcome!', 'woovator-pro' );
        }
        $name = apply_filters( 'woovator_profile_name', $name );
        ?>
            <div class="woovator-user-area">
                <div class="woovator-user-image">
                    <?php echo get_avatar( $current_user->user_email, 125 ); ?>
                </div>
                <div class="woovator-user-info">
                    <span class="woovator-username"><?php echo esc_attr( $name ); ?></span>
                    <span class="woovator-logout"><a href="<?php echo esc_url( wp_logout_url( get_permalink() ) ); ?>"><?php echo esc_html__( 'Logout', 'woovator-pro' ); ?></a></span>
                </div>
            </div>
        <?php
